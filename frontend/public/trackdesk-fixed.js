/**
 * Trackdesk CDN Tracking Script - Fixed Version
 * Version: 1.0.1
 * 
...
(function() {
  ...
  const TRACKDESK_CONFIG = {
    apiUrl: 'http://localhost:3002/api', // ← CHANGE TO YOUR API URL
    version: '1.0.1',
  ...
  const tracker = {
    ...
    init: (config = {}) => {
      ...
      const script = document.querySelector('script[src*="trackdesk"]');
      if (script) {
    ...
    sendEvents: async (events) => {
      ...
      utils.log('Sending events to server', payload);
