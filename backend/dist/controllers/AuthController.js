"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.AuthController = void 0;
var AuthService_1 = require("../services/AuthService");
var EmailService_1 = require("../services/EmailService");
var SecurityService_1 = require("../services/SecurityService");
var auth_1 = require("../middleware/auth");
var zod_1 = require("zod");
var authService = new AuthService_1.AuthService();
var emailService = new EmailService_1.EmailService();
var securityService = new SecurityService_1.SecurityService();
// Validation schemas
var registerSchema = zod_1.z.object({
    email: zod_1.z.string().email(),
    password: zod_1.z.string().min(8),
    firstName: zod_1.z.string().min(1),
    lastName: zod_1.z.string().min(1),
    role: zod_1.z.enum(["ADMIN", "AFFILIATE", "MANAGER"]).optional(),
});
var loginSchema = zod_1.z.object({
    email: zod_1.z.string().email(),
    password: zod_1.z.string().min(1),
});
var forgotPasswordSchema = zod_1.z.object({
    email: zod_1.z.string().email(),
});
var resetPasswordSchema = zod_1.z.object({
    token: zod_1.z.string(),
    password: zod_1.z.string().min(8),
});
var changePasswordSchema = zod_1.z.object({
    currentPassword: zod_1.z.string(),
    newPassword: zod_1.z.string().min(8),
});
var updateProfileSchema = zod_1.z.object({
    firstName: zod_1.z.string().min(1).optional(),
    lastName: zod_1.z.string().min(1).optional(),
    phone: zod_1.z.string().optional(),
    timezone: zod_1.z.string().optional(),
    language: zod_1.z.string().optional(),
});
var AuthController = /** @class */ (function () {
    function AuthController() {
    }
    AuthController.prototype.register = function (req, res) {
        return __awaiter(this, void 0, void 0, function () {
            var data, result, error_1;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 3, , 4]);
                        data = registerSchema.parse(req.body);
                        return [4 /*yield*/, authService.register(data)];
                    case 1:
                        result = _a.sent();
                        // Send welcome email
                        return [4 /*yield*/, emailService.sendWelcomeEmail(data.email, data.firstName)];
                    case 2:
                        // Send welcome email
                        _a.sent();
                        // Set authentication cookies
                        (0, auth_1.setAuthCookies)(res, result.token, result.user);
                        res.status(201).json({
                            message: "User created successfully",
                            token: result.token,
                            user: result.user,
                        });
                        return [3 /*break*/, 4];
                    case 3:
                        error_1 = _a.sent();
                        if (error_1.name === "ZodError") {
                            return [2 /*return*/, res
                                    .status(400)
                                    .json({ error: "Invalid input data", details: error_1.errors })];
                        }
                        res.status(400).json({ error: error_1.message });
                        return [3 /*break*/, 4];
                    case 4: return [2 /*return*/];
                }
            });
        });
    };
    AuthController.prototype.login = function (req, res) {
        return __awaiter(this, void 0, void 0, function () {
            var data, result, error_2;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        data = loginSchema.parse(req.body);
                        return [4 /*yield*/, authService.login(data, req.ip, req.get("User-Agent"))];
                    case 1:
                        result = _a.sent();
                        // Set authentication cookies
                        (0, auth_1.setAuthCookies)(res, result.token, result.user);
                        res.json({
                            message: "Login successful",
                            token: result.token,
                            user: result.user,
                        });
                        return [3 /*break*/, 3];
                    case 2:
                        error_2 = _a.sent();
                        if (error_2.name === "ZodError") {
                            return [2 /*return*/, res
                                    .status(400)
                                    .json({ error: "Invalid input data", details: error_2.errors })];
                        }
                        res.status(401).json({ error: error_2.message });
                        return [3 /*break*/, 3];
                    case 3: return [2 /*return*/];
                }
            });
        });
    };
    AuthController.prototype.logout = function (req, res) {
        return __awaiter(this, void 0, void 0, function () {
            var error_3;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        return [4 /*yield*/, authService.logout(req.user.id)];
                    case 1:
                        _a.sent();
                        // Clear authentication cookies
                        (0, auth_1.clearAuthCookies)(res);
                        res.json({ message: "Logout successful" });
                        return [3 /*break*/, 3];
                    case 2:
                        error_3 = _a.sent();
                        console.error("Logout error:", error_3);
                        res.status(500).json({ error: error_3.message });
                        return [3 /*break*/, 3];
                    case 3: return [2 /*return*/];
                }
            });
        });
    };
    AuthController.prototype.getProfile = function (req, res) {
        return __awaiter(this, void 0, void 0, function () {
            var user, error_4;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        return [4 /*yield*/, authService.getProfile(req.user.id)];
                    case 1:
                        user = _a.sent();
                        res.json(user);
                        return [3 /*break*/, 3];
                    case 2:
                        error_4 = _a.sent();
                        res.status(500).json({ error: error_4.message });
                        return [3 /*break*/, 3];
                    case 3: return [2 /*return*/];
                }
            });
        });
    };
    AuthController.prototype.updateProfile = function (req, res) {
        return __awaiter(this, void 0, void 0, function () {
            var data, user, error_5;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        data = updateProfileSchema.parse(req.body);
                        return [4 /*yield*/, authService.updateProfile(req.user.id, data)];
                    case 1:
                        user = _a.sent();
                        res.json(user);
                        return [3 /*break*/, 3];
                    case 2:
                        error_5 = _a.sent();
                        if (error_5.name === "ZodError") {
                            return [2 /*return*/, res
                                    .status(400)
                                    .json({ error: "Invalid input data", details: error_5.errors })];
                        }
                        res.status(500).json({ error: error_5.message });
                        return [3 /*break*/, 3];
                    case 3: return [2 /*return*/];
                }
            });
        });
    };
    AuthController.prototype.changePassword = function (req, res) {
        return __awaiter(this, void 0, void 0, function () {
            var data, error_6;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        data = changePasswordSchema.parse(req.body);
                        return [4 /*yield*/, authService.changePassword(req.user.id, data.currentPassword, data.newPassword)];
                    case 1:
                        _a.sent();
                        res.json({ message: "Password changed successfully" });
                        return [3 /*break*/, 3];
                    case 2:
                        error_6 = _a.sent();
                        if (error_6.name === "ZodError") {
                            return [2 /*return*/, res
                                    .status(400)
                                    .json({ error: "Invalid input data", details: error_6.errors })];
                        }
                        res.status(400).json({ error: error_6.message });
                        return [3 /*break*/, 3];
                    case 3: return [2 /*return*/];
                }
            });
        });
    };
    AuthController.prototype.forgotPassword = function (req, res) {
        return __awaiter(this, void 0, void 0, function () {
            var data, error_7;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        data = forgotPasswordSchema.parse(req.body);
                        return [4 /*yield*/, authService.forgotPassword(data.email)];
                    case 1:
                        _a.sent();
                        res.json({ message: "Password reset email sent" });
                        return [3 /*break*/, 3];
                    case 2:
                        error_7 = _a.sent();
                        if (error_7.name === "ZodError") {
                            return [2 /*return*/, res
                                    .status(400)
                                    .json({ error: "Invalid input data", details: error_7.errors })];
                        }
                        res.status(500).json({ error: error_7.message });
                        return [3 /*break*/, 3];
                    case 3: return [2 /*return*/];
                }
            });
        });
    };
    AuthController.prototype.resetPassword = function (req, res) {
        return __awaiter(this, void 0, void 0, function () {
            var data, error_8;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        data = resetPasswordSchema.parse(req.body);
                        return [4 /*yield*/, authService.resetPassword(data.token, data.password)];
                    case 1:
                        _a.sent();
                        res.json({ message: "Password reset successfully" });
                        return [3 /*break*/, 3];
                    case 2:
                        error_8 = _a.sent();
                        if (error_8.name === "ZodError") {
                            return [2 /*return*/, res
                                    .status(400)
                                    .json({ error: "Invalid input data", details: error_8.errors })];
                        }
                        res.status(400).json({ error: error_8.message });
                        return [3 /*break*/, 3];
                    case 3: return [2 /*return*/];
                }
            });
        });
    };
    AuthController.prototype.setup2FA = function (req, res) {
        return __awaiter(this, void 0, void 0, function () {
            var secret, error_9;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        return [4 /*yield*/, securityService.generate2FASecret(req.user.id)];
                    case 1:
                        secret = _a.sent();
                        res.json({ secret: secret });
                        return [3 /*break*/, 3];
                    case 2:
                        error_9 = _a.sent();
                        res.status(500).json({ error: error_9.message });
                        return [3 /*break*/, 3];
                    case 3: return [2 /*return*/];
                }
            });
        });
    };
    AuthController.prototype.verify2FA = function (req, res) {
        return __awaiter(this, void 0, void 0, function () {
            var token, isValid, error_10;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 5, , 6]);
                        token = req.body.token;
                        return [4 /*yield*/, securityService.verify2FAToken(req.user.id, token)];
                    case 1:
                        isValid = _a.sent();
                        if (!isValid) return [3 /*break*/, 3];
                        return [4 /*yield*/, authService.enable2FA(req.user.id)];
                    case 2:
                        _a.sent();
                        res.json({ message: "2FA enabled successfully" });
                        return [3 /*break*/, 4];
                    case 3:
                        res.status(400).json({ error: "Invalid token" });
                        _a.label = 4;
                    case 4: return [3 /*break*/, 6];
                    case 5:
                        error_10 = _a.sent();
                        res.status(500).json({ error: error_10.message });
                        return [3 /*break*/, 6];
                    case 6: return [2 /*return*/];
                }
            });
        });
    };
    AuthController.prototype.disable2FA = function (req, res) {
        return __awaiter(this, void 0, void 0, function () {
            var error_11;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        return [4 /*yield*/, authService.disable2FA(req.user.id)];
                    case 1:
                        _a.sent();
                        res.json({ message: "2FA disabled successfully" });
                        return [3 /*break*/, 3];
                    case 2:
                        error_11 = _a.sent();
                        res.status(500).json({ error: error_11.message });
                        return [3 /*break*/, 3];
                    case 3: return [2 /*return*/];
                }
            });
        });
    };
    AuthController.prototype.generateBackupCodes = function (req, res) {
        return __awaiter(this, void 0, void 0, function () {
            var codes, error_12;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        return [4 /*yield*/, authService.generateBackupCodes(req.user.id)];
                    case 1:
                        codes = _a.sent();
                        res.json({ codes: codes });
                        return [3 /*break*/, 3];
                    case 2:
                        error_12 = _a.sent();
                        res.status(500).json({ error: error_12.message });
                        return [3 /*break*/, 3];
                    case 3: return [2 /*return*/];
                }
            });
        });
    };
    return AuthController;
}());
exports.AuthController = AuthController;
