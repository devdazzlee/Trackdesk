...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    // For now, we'll return mock data since we don't have a ProgramUpdate model in Prisma
    const mockUpdates = [
      {
        id: "UPDATE-001",
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    ...
    // Mock data for individual update
    const mockUpdate = {
      id: "UPDATE-001",
      title: "New Commission Structure Effective March 1st",
      summary: "We're introducing a new tiered commission structure that rewards top performers with higher rates.",
...
router.post('/', async (req: Request, res: Response) => {
  try {
    const updateSchema = z.object({
      ...
      category: z.enum(['commission', 'marketing', 'system', 'policy', 'promotion', 'general']),
      priority: z.enum(['low', 'medium', 'high']),
      status: z.enum(['draft', 'published', 'archived']).default('draft'),
      tags: z.array(z.string()).default([]),
      publishAt: z.string().optional(),
      notifyAffiliates: z.boolean().default(true)
    });
...
router.put('/:id', async (req: Request, res: Response) => {
  try {
    ...
    const updateSchema = z.object({
      ...
      category: z.enum(['commission', 'marketing', 'system', 'policy', 'promotion', 'general']).optional(),
...
router.post('/:id/react', async (req: Request, res: Response) => {
  try {
    ...
    const reactionSchema = z.object({
      userId: z.string(),
      type: z.enum(['like', 'helpful', 'question'])
    });
