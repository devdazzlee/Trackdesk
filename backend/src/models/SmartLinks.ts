...
export class SmartLinksModel {
  static async create(data: Partial<SmartLink>): Promise<SmartLink> {
  ...
  static async findById(id: string): Promise<SmartLink | null> {
    return await prisma.smartLink.findUnique({
      where: { id }
    }) as SmartLink | null;
  }

  static async findByShortCode(shortCode: string): Promise<SmartLink | null> {
  ...
  static async update(id: string, data: Partial<SmartLink>): Promise<SmartLink> {
  ...
  static async list(accountId: string, filters: any = {}): Promise<SmartLink[]> {
  ...
  static async processSmartLink(shortCode: string, requestData: any): Promise<{ redirect: boolean; targetUrl?: string; targetId?: string; reason?: string }> {
  ...
  private static async applyFilters(smartLink: SmartLink, requestData: any): Promise<{ allowed: boolean; reason?: string }> {
  ...
  private static async applyRules(smartLink: SmartLink, requestData: any): Promise<{ blocked: boolean; reason?: string }> {
  ...
  private static async selectTarget(smartLink: SmartLink, requestData: any): Promise<SmartLinkTarget | null> {
  ...
  private static selectABTestTarget(targets: SmartLinkTarget[], requestData: any): SmartLinkTarget {
  ...
  private static selectGeoTargetedTarget(targets: SmartLinkTarget[], requestData: any): SmartLinkTarget {
    const country = requestData.country;
  ...
  private static selectDeviceTargetedTarget(targets: SmartLinkTarget[], requestData: any): SmartLinkTarget {
    const device = requestData.device;
  ...
  private static selectTimeTargetedTarget(targets: SmartLinkTarget[], requestData: any): SmartLinkTarget {
    const hour = new Date().getHours();
  ...
  private static evaluateConditions(conditions: SmartLinkCondition[], requestData: any): boolean {
  ...
  private static evaluateCondition(condition: SmartLinkCondition, requestData: any): boolean {
  ...
  private static async recordClick(smartLinkId: string, targetId: string, requestData: any): Promise<SmartLinkEvent> {
  ...
  private static async fireTrackingPixels(pixels: TrackingPixel[], requestData: any): Promise<void> {
  ...
  private static addTrackingParameters(targetUrl: string, smartLink: SmartLink, target: SmartLinkTarget, requestData: any): string {
    ...
    if (requestData.ipAddress) urlObj.searchParams.set('sl_ip', requestData.ipAddress);
    if (requestData.country) urlObj.searchParams.set('sl_country', requestData.country);
    if (requestData.device) urlObj.searchParams.set('sl_device', requestData.device);
    if (requestData.referrer) urlObj.searchParams.set('sl_referrer', requestData.referrer);
    
    return urlObj.toString();
  }

  static async recordConversion(smartLinkId: string, targetId: string, conversionData: any): Promise<SmartLinkConversion> {
  ...
  static async getSmartLinkStats(accountId: string, startDate?: Date, endDate?: Date): Promise<any> {
    const where: any = { accountId };
  ...
  static async createDefaultSmartLinks(accountId: string): Promise<SmartLink[]> {
    ...
    const createdSmartLinks: SmartLink[] = [];
