...
export class AuthorizationModel {
  ...
  static async findRoleByName(accountId: string, name: string): Promise<Role | null> {
  ...
  static async listRoles(accountId: string, filters: any = {}): Promise<Role[]> {
  ...
  static async assignRole(userId: string, roleId: string, accountId: string, grantedBy: string, expiresAt?: Date): Promise<UserRole> {
  ...
  static async getUserRoles(userId: string, accountId: string): Promise<UserRole[]> {
  ...
  static async checkPermission(userId: string, accountId: string, resource: string, action: string, resourceId?: string, context?: any): Promise<boolean> {
    // Get user roles
    ...
    for (const userRole of userRoles) {
      ...
      for (const permission of role.permissions) {
        if (permission.resource === resource && permission.action === action && permission.granted) {
  ...
  private static evaluatePermissionConditions(conditions: PermissionCondition[], context: any, resourceId?: string): boolean {
  ...
  private static evaluateAccessConditions(conditions: AccessCondition[], context: any, resourceId?: string): boolean {
    if (conditions.length === 0) return true;
  ...
  private static evaluateCondition(condition: PermissionCondition | AccessCondition, context: any, resourceId?: string): boolean {
    let value: any;
  ...
  static async createAccessControl(data: Partial<AccessControl>): Promise<AccessControl> {
  ...
  static async getAccessControls(userId: string, accountId: string, resource: string, resourceId?: string): Promise<AccessControl[]> {
    const where: any = {
      accountId,
      resource,
      status: 'ACTIVE'
  ...
  private static async getUserRoleIds(userId: string, accountId: string): Promise<string[]> {
    const userRoles = await this.getUserRoles(userId, accountId);
  ...
  static async getAuditLogs(accountId: string, filters: any = {}, page: number = 1, limit: number = 50): Promise<AuditLog[]> {
  ...
  static async revokeAllUserSessions(userId: string, accountId: string): Promise<void> {
  ...
  static async createTwoFactorAuth(data: Partial<TwoFactorAuth>): Promise<TwoFactorAuth> {
  ...
  static async findTwoFactorAuthByUser(userId: string, accountId: string): Promise<TwoFactorAuth | null> {
  ...
  static async updateTwoFactorAuth(id: string, data: Partial<TwoFactorAuth>): Promise<TwoFactorAuth> {
  ...
  static async verifyTwoFactorCode(userId: string, accountId: string, code: string): Promise<boolean> {
    ...
    const defaultRoles = [
      {
        ...
        permissions: [
          { id: 'all', resource: '*', action: '*', granted: true }
        ],
        isSystem: true,
        isDefault: true
      },
      {
        ...
        permissions: [
          { id: 'affiliates_manage', resource: 'affiliates', action: 'manage', granted: true },
          { id: 'offers_manage', resource: 'offers', action: 'manage', granted: true },
          { id: 'reports_view', resource: 'reports', action: 'view', granted: true },
          { id: 'settings_manage', resource: 'settings', action: 'manage', granted: true }
        ],
        isSystem: false,
        isDefault: true
      },
      {
        ...
        permissions: [
          { id: 'affiliates_view', resource: 'affiliates', action: 'view', granted: true },
          { id: 'affiliates_edit', resource: 'affiliates', action: 'edit', granted: true },
          { id: 'offers_view', resource: 'offers', action: 'view', granted: true },
          { id: 'reports_view', resource: 'reports', action: 'view', granted: true }
        ],
        isSystem: false,
        isDefault: true
      },
      {
        ...
        permissions: [
          { id: 'dashboard_view', resource: 'dashboard', action: 'view', granted: true },
          { id: 'links_manage', resource: 'links', action: 'manage', granted: true },
          { id: 'reports_view_own', resource: 'reports', action: 'view', conditions: [{ field: 'affiliateId', operator: 'OWNER', value: null }], granted: true }
        ],
        isSystem: false,
        isDefault: true
      }
