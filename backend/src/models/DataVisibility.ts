...
export class DataVisibilityModel {
  static async createRule(data: Partial<DataVisibilityRule>): Promise<DataVisibilityRule> {
  ...
  static async update(id: string, data: Partial<DataVisibilityRule>): Promise<DataVisibilityRule> {
  ...
  static async list(accountId: string, filters: any = {}): Promise<DataVisibilityRule[]> {
  ...
  static async checkAccess(userId: string, resourceType: string, resourceId: string, action: string, userRole: string, affiliateId?: string): Promise<{ allowed: boolean; reason?: string; maskedFields?: string[] }> {
  ...
  private static async evaluateConditions(conditions: VisibilityCondition[], resourceType: string, resourceId: string): Promise<boolean> {
  ...
  private static async evaluateCondition(condition: VisibilityCondition, resourceType: string, resourceId: string): Promise<boolean> {
    // This would need to be implemented based on the specific resource type
    // For now, return true as a placeholder
    return true;
  }

  static async logAccess(userId: string, resourceType: string, resourceId: string, action: string, ipAddress: string, userAgent: string, success: boolean, reason?: string): Promise<DataAccessLog> {
  ...
  static async getAccessLogs(filters: any = {}, page: number = 1, limit: number = 50): Promise<DataAccessLog[]> {
  ...
  static async createMaskingRule(data: Partial<DataMaskingRule>): Promise<DataMaskingRule> {
  ...
  static async findMaskingRuleById(id: string): Promise<DataMaskingRule | null> {
    return await prisma.dataMaskingRule.findUnique({
      where: { id }
    }) as DataMaskingRule | null;
  }

  static async updateMaskingRule(id: string, data: Partial<DataMaskingRule>): Promise<DataMaskingRule> {
  ...
  static async listMaskingRules(accountId: string, filters: any = {}): Promise<DataMaskingRule[]> {
    const where: any = { accountId };
  ...
  static async applyMasking(data: any, userId: string, userRole: string): Promise<any> {
    const user = await prisma.user.findUnique({
      where: { id: userId }
    });
  ...
  static async getDataVisibilityStats(accountId: string, startDate?: Date, endDate?: Date): Promise<any> {
    const where: any = {};
  ...
  static async createDefaultRules(accountId: string): Promise<DataVisibilityRule[]> {
    const defaultRules = [
      {
  ...
  static async createDefaultMaskingRules(accountId: string): Promise<DataMaskingRule[]> {
    const defaultRules = [
      {
        field: 'ssn',
  ...
  static async importDataVisibilityConfig(accountId: string, config: any): Promise<void> {
    // Import rules
    if (config.rules) {
